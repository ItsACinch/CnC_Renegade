...
// Modern C++ compliant version - requires class name parameter
#define ADD_STATE_TO_MACHINE(machine, state)		\
		machine.Add_State (								\
			&ThisClass::On_##state##_Think,			\
			&ThisClass::On_##state##_Request_End,	\
			&ThisClass::On_##state##_Begin,			\
			&ThisClass::On_##state##_End);
