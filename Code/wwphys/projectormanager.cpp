		...
		if (Projector != NULL) {
			...
			*/
			if (def.IsPerspective) {
				Projector->Set_Perspective_Projection(	def.HorizontalFOV,
																	def.VerticalFOV,
																	def.NearZ,
																	def.FarZ);
			} else {
				Projector->Set_Ortho_Projection(			-def.OrthoWidth * 0.5f,def.OrthoWidth * 0.5f,
																	-def.OrthoHeight * 0.5f,def.OrthoHeight * 0.5f,
																	def.NearZ,def.FarZ);
			}
